...
def clean_real_estate_data(df):
    ...
    boolean_cols = ['hasOffice', 'hasSwimmingPool', 'hasFireplace', 'hasTerrace', 
                   'accessibleDisabledPeople','hasAttic', 'hasGarden', 'hasAirConditioning', 
                      'hasArmoredDoor', 'hasVisiophone', 'hasTerrace',
                      'hasOffice', 'hasSwimmingPool', 'hasFireplace',
...
def encode_categorical_features(df):
    ...
    print(f"Province encoding: {df_encoded['province_encoded'].value_counts().sort_index().to_dict()}")
    ...
    if 'locality' in df_encoded.columns and df_encoded['locality'].notna().sum() > 0:
        ...
        df_encoded.loc[mask, 'locality_encoded'] = le_locality.fit_transform(df_encoded.loc[mask, 'locality'])
        print(f"Locality encoded: {df_encoded['locality_encoded'].notna().sum()} localities")
    else:
        le_locality = None
    ...
    if 'gardenOrientation' in df_encoded.columns:
        garden_orientation_mapping = {
            'SOUTH': 4,
            'SOUTH_WEST': 3, 'SOUTHWEST': 3,
            'SOUTH_EAST': 3, 'SOUTHEAST': 3,
            'WEST': 2,
            'EAST': 2,
            'NORTH_WEST': 1, 'NORTHWEST': 1,
            'NORTH_EAST': 1, 'NORTHEAST': 1,
            'NORTH': 5,
            'UNKNOWN': 0
        }
        df_encoded['gardenOrientation_encoded'] = df_encoded['gardenOrientation'].map(garden_orientation_mapping)
        print(f"Garden orientation encoded: {df_encoded['gardenOrientation_encoded'].value_counts().to_dict()}")
    
    # 6. TERRACE ORIENTATION ENCODING (if exists)
    print("\n Encoding terrace orientation...")
    if 'terraceOrientation' in df_encoded.columns:
        df_encoded['terraceOrientation_encoded'] = df_encoded['terraceOrientation'].map(garden_orientation_mapping)
        print(f"Terrace orientation encoded: {df_encoded['terraceOrientation_encoded'].value_counts().to_dict()}")
    ...
    if 'epcScore' in df_encoded.columns:
        ...
        print(f"EPC encoding: {df_encoded['epcScore_encoded'].value_counts().sort_index().to_dict()}")
    
    # 8. BOOLEAN FEATURES - Convert True/False to 1/0
    print("\n Encoding boolean features...")
    boolean_columns = ['hasAttic', 'hasGarden', 'hasAirConditioning', 
                      'hasArmoredDoor', 'hasVisiophone', 'hasTerrace',
                      'hasOffice', 'hasSwimmingPool', 'hasFireplace',
                      'accessibleDisabledPeople']
    ...
    for col in boolean_columns:
        if col in df_encoded.columns:
            df_encoded[f'{col}_encoded'] = df_encoded[col].map({True: 1, False: 0, np.nan: 0})
            print(f"{col} encoded: {df_encoded[f'{col}_encoded'].value_counts(dropna=False).to_dict()}")
    ...
    return df_encoded, {
        ...
        'garden_orientation_mapping': garden_orientation_mapping if 'gardenOrientation' in df_encoded.columns else None,
...
def preprocess_missing_values(df):
    ...
    if 'gardenOrientation' in df_processed.columns:
        df_processed['gardenOrientation'] = df_processed['gardenOrientation'].fillna('UNKNOWN')
        print("Filled missing garden orientations with 'UNKNOWN'")
    
    if 'terraceOrientation' in df_processed.columns:
        df_processed['terraceOrientation'] = df_processed['terraceOrientation'].fillna('UNKNOWN')
    ...
    boolean_cols = ['hasAttic', 'hasGarden', 'hasAirConditioning', 
                   'hasArmoredDoor', 'hasVisiophone', 'hasTerrace',
                   'hasOffice', 'hasSwimmingPool', 'hasFireplace',
                   'accessibleDisabledPeople']
...
def create_final_ml_dataset(df_encoded):
    ...
    numeric_features = ['bedroomCount', 'bathroomCount', 'habitableSurface', 
                       'toiletCount', 'terraceSurface', 'postCode']
    
    encoded_features = ['province_encoded', 'type_encoded', 'subtype_encoded', 
                       'locality_encoded', 'gardenOrientation_encoded', 
                       'terraceOrientation_encoded', 'epcScore_encoded']
    
    boolean_features = ['hasAttic_encoded', 'hasGarden_encoded', 'hasAirConditioning_encoded',
